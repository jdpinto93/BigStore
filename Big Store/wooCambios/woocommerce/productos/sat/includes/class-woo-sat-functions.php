...
if( !class_exists( 'Woo_SAT_Functions' ) ) {

    /**
     * Woo_SAT_Functions class
     *
     * @since       0.2.0
     */
    class Woo_SAT_Functions {

        /**
         * @var         Woo_SAT_Functions $instance The one true Woo_SAT_Functions
         * @since       0.2.0
         */
         ...
         * @access      public
         * @since       0.2.0
         * @return      object self::$instance The one true Woo_SAT_Functions
         */
        public static function instance() {
            if( !self::$instance ) {
                self::$instance = new Woo_SAT_Functions();
                self::$instance->hooks();
            }
        ...
        private function hooks() {
            ...
            add_filter( 'woocommerce_structured_data_product', array( $this, 'add_sat_to_structured_data' ), 10, 2 );
        ...
        public function scripts_styles( $hook ) {
            ...
            wp_enqueue_script( 'woo-add-sat', Woo_SAT_URL . 'assets/js/woo-add-sat' . $suffix . '.js', array( 'wc-add-to-cart-variation', 'jquery' ), Woo_SAT_VER, true );

            $localized = array(
                "sat" => get_post_meta( $post->ID, 'hwp_product_sat', 1 )
            );
            ...
            if( !empty( $variations ) ) {
                ...
                foreach ( $variations as $variation ) {
                    if( !empty( $variation ) && $variation['variation_is_active'] != false ) {
                        ...
                        $localized["variation_sats"][$variation['variation_id']] = get_post_meta( $variation['variation_id'], 'hwp_var_sat', 1 );
                ...
                foreach ( $composite_variations as $id => $comp_variation ) {
                    ...
                    foreach ( $comp_variation as $variation ) {
                        if( !empty( $variation ) && $variation['variation_is_active'] != false ) {
                            ...
                            $localized["composite_variation_sats"][$variation['variation_id']] = get_post_meta( $variation['variation_id'], 'hwp_var_sat', 1 );
            ...
            wp_localize_script( 'woo-add-sat', 'wooSatVars', $localized );
        ...
        public function maybe_display_tn() {
            ...
            global $post;
            $sat = get_post_meta( $post->ID, 'hwp_product_sat', 1 );
            $display = get_option( 'hwp_display_sat' );
            $label = ( !empty( get_option( 'hwp_sat_text' ) ) ? get_option( 'hwp_sat_text' ) : 'SAT' );
            ...
            if( !empty( $sat ) ) {
                ...
                echo '<span class="hwp-sat">' . esc_html__( $label . ': ', 'woo-add-sat' ) . '<span>' . get_post_meta( $post->ID, 'hwp_product_sat', 1 ) . '</span></span>';
        ...
        /**
         * Adds SAT field to product JSON-LD.
         *
         * @since       0.5.0
         * @return      array    Modified WooCommerce product structured data.
         */
        public function add_sat_to_structured_data( $markup, $product ) {
		
            // Bail if product variable not available
            if ( ! $product ) { return $markup; }
            
            $product_id = $product->get_id();
            $sat = get_post_meta( $product_id, 'hwp_product_sat', 1 );
            $markup['sat'] = trim($sat);
            return $markup;
    ...
    $Woo_SAT_Functions = new Woo_SAT_Functions();
    $Woo_SAT_Functions->instance();
