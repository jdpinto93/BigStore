...
class WC_Order_Status_Manager_Admin_Order_Statuses {
	...
	public function order_status_columns( $columns ) {
		...
		$columns['description'] = __( 'Descripcion', 'woocommerce-order-status-manager' );
		$columns['paid']        = __( 'Pagado', 'woocommerce-order-status-manager' );
		$columns['reports']     = __( 'Reportes', 'woocommerce-order-status-manager' );
		$columns['type']        = __( 'Tipo', 'woocommerce-order-status-manager' );

		$first_column = array( 'icon' => __( 'Icono', 'woocommerce-order-status-manager' ) );
			...
			<li class="wide">
				<div id="delete-action">
					<?php if ( ! $status->is_core_status() && current_user_can( "delete_post", $post->ID ) ) : ?>
						<a class="submitdelete deletion" href="<?php echo esc_url( get_delete_post_link( $post->ID, '', true ) ); ?>"><?php esc_html_e( 'Eliminar', 'woocommerce-order-status-manager' ); ?></a>
					<?php endif; ?>
				</div>
	...
	public function reassign_order_status_popup() {
		...
		if ( 'wc_order_status' === $typenow ) :
			...
			<div id="reassign-order-status-popup" style="display: none;">
				...
				<button class="button delete"><?php esc_html_e( 'Eliminar', 'woocommerce-order-status-manager' ); ?></button>
