...
class WC_Order_Status_Manager_Admin {
	...
	public function print_settings_tabs() {
		...
		?>
		<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="nav-tab <?php echo ( 'wc_order_status' === $typenow ? 'nav-tab-active' : '' ); ?>"><?php esc_html_e( 'Estados de Pedido', 'woocommerce-order-status-manager' ); ?></a>
		<?php
	}
	...
	public function output_sections() {
		...
		?>
		<ul class="subsubsub">
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="<?php echo ( 'wc_order_status' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Estados de Pedido', 'woocommerce-order-status-manager' ); ?></a> | </li>
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_email' ) ); ?>" class="<?php echo ( 'wc_order_email' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Correos', 'woocommerce-order-status-manager' ); ?></a></li>
		</ul>
		<br class="clear" />
		<?php
	}
	...
	public function normalize_columns( $columns ) {
		...
		$columns['title'] = __( 'Nombre', 'woocommerce-order-status-manager' );
		...
		if ( $this->is_order_status_manager_post_type( get_post_type( $post->ID ) ) ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) && ! $order_status->is_core_status() ) {
				...
				$actions['delete'] = sprintf(
					...
					__( 'Eliminar', 'woocommerce-order-status-manager' )
				);
			}
		}
